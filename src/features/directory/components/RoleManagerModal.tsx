...
export const RoleManagerModal: React.FC<RoleManagerModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[850px] p-0 overflow-hidden bg-sidebar border-border gap-0">
        <div className="flex h-[700px]">
          ...
          <div className="w-[240px] border-r border-border bg-sidebar/50 flex flex-col">
            <div className="h-14 px-4 border-b border-border flex items-center">
              <h3 className="text-foreground font-medium flex items-center gap-2">
                <Settings className="w-4 h-4 text-primary" /> {t('directory.role_manager.title')}
              </h3>
            </div>
            ...
            <div className="p-4 border-t border-border">
              <Button
                variant="outline"
                className="w-full text-xs h-9 bg-background hover:bg-muted border-border"
                onClick={() => {
                  setEditingRole('new');
          ...
          <div className="flex-1 flex flex-col bg-sidebar">
            ...
            {(editingRole === 'new' || (editingRole && editingRole.id)) ? (
              <>
                <div className="flex-1 p-6 overflow-y-auto space-y-6 scrollbar-dark">
                  <div>
              ...
              <div className="flex-1 flex items-center justify-center text-muted-foreground flex-col">
                ...
                <Button variant="outline" className="mt-6 bg-background border-border" onClick={onClose}>{t('directory.role_manager.close_manager')}</Button>
              </div>
            )}
          </div>
        </div>
