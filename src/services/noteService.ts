...
export const noteService = {
  ...
  async getNoteTeams(workspaceId?: string) {
    ...
    return teams
      .map((team) => {
        ...
        const workspaces = team.workspaces as { name: string } | { name: string }[] | null | undefined;
  ...
  async getTeamNotes(teamId: string) {
    const { data, error } = await supabase
      .from('work_notes')
      .select('id, team_id, workspace_id, author_id, subject, content, created_at, edit_history, author:users(full_name, email)')
      .eq('team_id', teamId)
      .order('created_at', { ascending: false });
  ...
  async createNote(payload: {
    ...
    const { error } = await supabase.from('work_notes').insert(payload);
  ...
  async updateNote(noteId: string, payload: {
    ...
    const { error } = await supabase.from('work_notes').update(payload).eq('id', noteId);
