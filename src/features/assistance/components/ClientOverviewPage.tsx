   ...
   if (!client) {
      return (
         <div className="p-8 max-w-4xl mx-auto h-full flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-xl font-bold mb-2">{t('assistance.no_client_linked', 'Ingen Brukare Kopplad')}</h2>
            <p className="text-muted-foreground max-w-md mx-auto">
               {t('assistance.no_client_description', 'Detta team (arbetslag) har ingen brukare associerad till sig. En chef kan lägga till och koppla en brukare till teamet via katalogen (Directory).')}
            </p>
         </div>
      );
   }
      ...
      <div className="flex-1 overflow-y-auto w-full h-full p-8 max-w-5xl mx-auto">
         <div className="bg-sidebar border border-border shadow-sm rounded-xl mb-6 p-6 flex items-start gap-6">
            ...
            <div>
               ...
               <p className="text-muted-foreground mt-1 text-sm font-medium">{t('assistance.personal_number', 'Personnummer:')} {client.personal_number || t('common.unknown', 'Okänt')}</p>
               <div className="mt-4 flex gap-2">
                  <span className="px-3 py-1 bg-secondary border border-border rounded-full text-[11px] font-bold uppercase tracking-wider text-muted-foreground">{t('assistance.care_level', 'Omvårdnadsnivå:')} {client.care_level}</span>
               </div>
            </div>
         </div>
         ...
         <div className="flex gap-2 border-b border-border mb-6">
            ...
            >
               <div className="flex gap-2 items-center"><FileText className="w-4 h-4" /> {t('assistance.daily_notes', 'Daganteckningar')}</div>
            </button>
            <button
            ...
            >
               <div className="flex gap-2 items-center"><Pill className="w-4 h-4" /> {t('assistance.active_medication_list', 'Aktiv Medicinlista')}</div>
            </button>
         </div>
